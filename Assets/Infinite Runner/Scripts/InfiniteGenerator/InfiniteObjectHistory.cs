    ...
    public class InfiniteObjectHistory : MonoBehaviour
    {
        ...
        public void LoadInfiniteObjectPersistence(InfiniteObjectPersistence persistence)
        {
            ...
            for (int i = 0; i < (int)ObjectLocation.Last; ++i) {
                ...
                /*
                for (int j = 0; j < objectCount; ++j) {
                    objectSpawnIndex[i][j] = persistence.objectSpawnIndex[j * width + i];
                    lastObjectSpawnDistance[i][j] = persistence.lastObjectSpawnDistance[j * width + i];
                }
                for (int j = 0; j < (int)ObjectType.Last; ++j) {
                    objectTypeSpawnIndex[i][j] = persistence.objectTypeSpawnIndex[j * width + i];
                    lastLocalIndex[i][j] = persistence.lastLocalIndex[j * width + i];
                    latestObjectTypeSpawnDistance[i][j] = persistence.latestObjectTypeSpawnDistance[j * width + i];
                }
                 **/
                for (int j = 0; j < objectCount; ++j) {
